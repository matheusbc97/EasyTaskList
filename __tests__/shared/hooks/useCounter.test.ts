...
describe('useCounter Hook', () => {
  ...
  it('Should reset counter at 0 after minValue', () => {
    const {result} = renderHook(() =>
      useCounter({minValue: -1, isCountdown: true}),
    );
  ...
  it('Should reset counter at maxValue after minValue', () => {
    const maxValue = 3;
    const {result} = renderHook(() =>
      useCounter({minValue: -1, isCountdown: true, maxValue}),
    );
